...
jQuery.catTopLoad = function(tpl) {
    var load_num = 0;
    var execute = true;
    var $minUl = $("*[shoptype='catetopWarp']");
    var wrapHeight = $minUl.height();
    var wrapTop = $minUl.offset().top;
    var items = $("*[shoptype='items']").find("*[shoptype='item']");
        ...
        if (execute && key < items.length) {
            $.ajax({
                ...
                success: function(data) {
                    $("*[shoptype='floor-loading']").hide();
                    load_brand_response(data);
                },
                beforeSend: function() {
                    $("*[shoptype='floor-loading']").show();
                }
            });
            execute = false;
        }
    ...
    function load_brand_response(result) {
        if (!result.error) {
            load_num += 1;
            $("*[shoptype='goods_cat_level']").append(result.content);
...
jQuery.homefloorLoad = function() {
    var load_num = 0;
    var execute = true;
    var wrapTop = $("*[shoptype='goods_cat_level']").position().top;
...
jQuery.itemLoad = function(obj, it, best, query_string, model) {
    var execute = true
        ...
        offsetHeight = 0
        , //容器距离顶部高度
        loading = $("*[shoptype='floor-loading']");
    //判断it值是否存在
    if (!it)
        it = "li";
    $(window).on("scroll", function() {
    ...
    function load_more_goods(count, sec_count) {
        $.ajax({
            ...
            success: function(data) {
                loading.hide();
                if (data.cat_goods) {
                    obj.find("*[shoptype='items']").append(data.cat_goods);
                    if (data.best_goods) {
                        $(best).find("ul").append(data.best_goods);
                    }
                    if ($.goodsAjaxLoadType) {
                        $.goodsAjaxLoadType()
                    }
                    ;execute = true;
                }
...
jQuery.catetopLift = function() {
    // dom
    var lift = $("*[shoptype='lift']");
    var lift_item = lift.find("*[shoptype='liftItem']");
    var floor_wp = $("*[shoptype='goods_cat_level']");
    var floorItem = floor_wp.find("*[shoptype='floorItem']");
    ...
    $(window).scroll(init);
    // click
    $(document).on("click", "*[shoptype='liftItem']", function() {
        var $this = $(this);
...
$.scrollLoad = function(main, list, child, info) {
    ...
    $(document).on('scroll', window, function() {
        ...
        //窗口高度
        var ot = $(main).offset().top;
        //上边距
        var oh = $(main).outerHeight();
        //总高度
        if ((st + wh) > (ot + oh)) {
            if (execute) {
                execute = false;
                //异步操作
                var cat_id = $("*[shoptype='brandCate'] a.curr").data("catid");
                //获取分类id（自定义部分，可修改）
