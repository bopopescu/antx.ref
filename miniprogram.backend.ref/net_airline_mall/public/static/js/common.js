...
function collect(goodsId) {
    ...
    Ajax.call('/home/goods/favorite', 'id=' + goodsId, collectResponse, 'POST', 'JSON');
}
...
function collectResponse(result) {
    ...
    var content = '<div id="flow_add_cart">' +
        ...
        '<div class="item-fore">' +
        '<h3 class="rem ftx-04">' + result.msg + '</h3>' +
        '</div>' +
        '</div>' +
        '</div>';
    pb({
        ...
        onCancel: function () {
            location.href = "/home/member/favorite";
        }
    });
    $('#' + divId + ' .item-fore').css({
        'padding-top': '12px'
